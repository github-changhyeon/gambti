...
object Gambti {
	...
	def calcMbtiRating(user: DataFrame, game: DataFrame): DataFrame = {
		val genre_df = getTable("genre")
		val game_genre_df = getTable("game_genre")
	
		//전처리 : 계산에 필요한 정보만 추출
		val user_info = user.select('no, 'age, 'gender, 'max_price, 'mbti)
		val game_info = game.select('game_id, 'metascore)
        ...
        //mbti별 steam 장르에 해당하는 계산 결과값
		return final_mgenre_rating
	}	

    def calcmGameRating(game_genre: DataFrame, final_mgenre_rating: DataFrame): DataFrame = {
        val game_genre_df = game_genre.drop('id).withColumnRenamed("genre_id", "sgenre_id").orderBy('game_id)
		...
		return final_mgame_rating
    }
    
	def calcGenderRating(game_genre_df: DataFrame): DataFrame = {
		/* 성별에 따른 rating */
		...
		return final_gender_rating
	}

	def calcAgeRating(game_genre_df: DataFrame): DataFrame = {
		/* 연령대에 따른 rating */
		...
		return final_age_rating
	}

	def makeUserGameMatrix(user: DataFrame, game: DataFrame, final_mgame_rating: DataFrame, final_gender_rating: DataFrame, final_age_rating: DataFrame): DataFrame = {
		//전처리 : 계산에 필요한 정보만 추출
		val user_info = user.select('no, 'age, 'gender, 'max_price, 'mbti)
		val game_info = game.select('game_id, 'metascore)
	...
	def runAlsModel(user_game: DataFrame): DataFrame = {
		/* als model */
	...
	def main(args: Array[String]){
		...
		val mbti_rating_df = calcMbtiRating(user_df, game_df)

		val game_genre_df = getTable("game_genre").drop('id).withColumnRenamed("genre_id", "sgenre_id").orderBy('game_id)

        val mgame_rating_df = calcmGameRating(game_genre_df, mbti_rating_df)
		
		val gender_rating_df = calcGenderRating(game_genre_df)
