...
export default function ChatList({ showChat }) {
  ...
  useEffect(() => {
    return fire.db.collection("rooms").orderBy("timestamp").onSnapshot((docs) => {
  ...
  const ReadChats = async (roomIds) => {
    ...
    const roomInfos = (await Promise.all(roomInfosPromise)).filter(
        ...
        {chatList.length === 0 ? (
          ...
          <div>
            {chatList.map((room, i) => {
              if (room.type === 'OneOnOne') {
                return (
                  <div key={i} className={styles.lists} style={{ width: '195px' }}>
                    <MediumProfile
                      propsUser={{
                    ...
                    {newChatsIds.includes(room.roomId) ? (
                      // <ButtonComp size="xsmall" textvalue="ee" color="#CCFF00" />
                      <FiberNewIcon className={styles.new_button} color="error" />
                    ) : (
                      <></>
                    )}
                    <hr />
