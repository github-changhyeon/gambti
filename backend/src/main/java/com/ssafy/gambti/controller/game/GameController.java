...
public class GameController {
    ...
    public ResponseEntity<? extends Response> gameDetail(@PathVariable Long gameId){
        ...
        Optional<Game> game = gameService.gameDetail(gameId);

        if (game != null) {
            Game data = game.orElseGet(Game::new);
            return new ResponseEntity<>(new Response(SUCCESS, "게임 디테일 조회 성공", null), HttpStatus.OK);
        } else {
